 0}, {124, 4, 24, 4, 120}, {124, 8, 4, 4, 120}, {56, 68, 68, 68, 56}, {252, 36, 36, 36, 24}, {24, 36, 36, 40, 252}, {124, 8, 4, 4, 8}, {72, 84, 84, 84, 32}, {4, 63, 68, 64, 32}, {60, 64, 64, 32, 124}, {28, 32, 64, 32, 28}, {60, 64, 48, 64, 60}, {68, 40, 16, 40, 68}, {28, 160, 160, 160, 124}, {68, 100, 84, 76, 68}, {8, 54, 65, 65, 0}, {0, 0, 119, 0, 0}, {0, 65, 65, 54, 8}, {2, 1, 2, 4, 2}, {124, 68, 68, 68, 124}};
...
void SSD1306Lite::drawTextCentered(
    int y,
    const char *str,
    int scale,
    DrawMode mode)
...
void SSD1306Lite::drawCharClipped(
    int x,
    int y,
    char c,
    int scale,
...
{
    int charWidth = 6 * scale;
    int charHeight = 9 * scale;
    int visibleLines = h / charHeight;

    //
    // PASS 1 - Count wrapped lines
    //
    auto countLines = [&](int textWidth)
    {
        int lines = 1;
        int cursorX = x;
        ...
        while (*str)
        {
            ...
            int wordWidth = wordLen * charWidth;
            ...
            if (wordWidth > textWidth)
            {
                ...
                {
                    if (cursorX + charWidth > x + textWidth)
                    {
                        cursorX = x;
                        lines++;
                    }
            ...
            {
                if (cursorX + wordWidth > x + textWidth)
                {
                    cursorX = x;
                    lines++;
                }
    ...
    int totalLines = countLines(w);

    bool showScrollBar = scrollBar && (totalLines > visibleLines);

    int textWidth = showScrollBar ? w - 3 : w;

    // Recount with scrollbar space removed
    if (showScrollBar)
        totalLines = countLines(textWidth);
    ...
    while (*str)
    {
        ...
        if (wordWidth > textWidth)
        {
            for (int i = 0; i < wordLen; i++)
            {
                ...
                if (currentLine >= firstLine &&
                    currentLine < firstLine + visibleLines)
                {
                    drawCharClipped(
                        cursorX,
                        y + (currentLine - firstLine) * charHeight,
                        str[i],
                        scale,
                        x,
                        y,
        ...
        else
        {
            ...
            {
                int drawX = cursorX;
                int drawY = y + (currentLine - firstLine) * charHeight;
    ...
    if (showScrollBar)
    {
        int thumbHeight = max(4, (visibleLines * h) / totalLines);

        int thumbY = y;

        if (maxScroll > 0)
            thumbY += (firstLine * (h - thumbHeight)) / maxScroll;

        for (int py = y; py < y + h; py++)
        {
            if (py >= thumbY &&
                py < thumbY + thumbHeight)
            {
                setPixel(x + w - 3, py, mode);
                setPixel(x + w - 2, py, mode);
                setPixel(x + w - 1, py, mode);
            }
            else
            {
                setPixel(x + w - 1, py, mode);
